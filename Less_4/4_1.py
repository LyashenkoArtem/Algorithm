'''
1. Проанализировать скорость и сложность одного - трёх
 любых алгоритмов, разработанных в рамках домашнего задания первых трех уроков.
'''
...
''' 
Итог: 
сложность данного алгоритма так же О(N**2), 
при этом на небольшом размере матрицы (10, 10) он медленнее 
алгоритмов №1 и №2, но на размере 100 и >, он выигрывает по 
времени у алгоритма №2, но для таких значений требуется 
увеличения стека для рекурсии, так как при стандартном размере 
стек переполняется, не давая алгоритму завершить действие.

По итогу проведения оценки скорости работы алгоритмов, с 
уверенностью можно сказать, что наиболее выгодным алгоритмом
